...
@Controller
@RequestMapping("redpacket/voucherSend")
public class RedPacketVoucherSendController extends BaseController {
    ...
    @RequestMapping(value = "list", method = { RequestMethod.GET,
            RequestMethod.POST })
    @RequiresPermissions("vouchersend_list")
    public String pageQuery(
            @RequestParam(value = "pageSize", defaultValue = PAGE_SIZE) int pageSize,
            @RequestParam(value = "pageCurrent", defaultValue = PAGE_CURRENT) int pageCurrent,
            @ModelAttribute("condition") String condition,
            @ModelAttribute("interestCondition") RedPacketVoucherCondition voucherCondition, Model model) {
        ...
        return "redpacket/vouchersend/list";
    }
    ...
    @RequiresPermissions("vouchersend_info")
    public String findRedPaperById(@PathVariable("id") Long id,
        ...
        userRedPaperCondition.setRedpacketType((byte)3);
        ...
        return "redpacket/vouchersend/info";
    }
    ...
    @RequiresPermissions("vouchersend_give")
    public String send(Model model) {
        model.addAttribute("vipGrades", vipGradeService.findAll(new Sort(Sort.Direction.ASC, "id")));
        model.addAttribute("redpackets", voucherService.findAll(RedPacketInterest.STATE_INUSE));
        return "redpacket/vouchersend/send";
    }
    ...
    public String sendRedpacket(Long id, HttpSession session, Model model) throws Exception {
        try {
            ...
            // 发送红包
            String result = userRedPacketService.sendRedpacketVoucher(rpUser, id);
            session.setAttribute("message", result);
            return "redirect:/redpacket/voucherSend/list";
        } catch (Exception e) {
            e.printStackTrace();
            model.addAttribute("redpackets", voucherService.findAll(RedPacketInterest.STATE_INUSE));
