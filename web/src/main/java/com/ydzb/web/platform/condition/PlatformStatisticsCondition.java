...
public class PlatformStatisticsCondition {
    ...
    public List<SearchFilter> getAndFilters() {
        ...
        if (totalStartTime != null && !"".equals(totalStartTime)) {
            filters.add(SearchFilterHelper.newCondition("totalTime", SearchOperator.gte, DateUtil.getSystemTimeDay(totalStartTime)));
        }

        if (totalEndTime != null && !"".equals(totalEndTime)) {
            filters.add(SearchFilterHelper.newCondition("totalTime", SearchOperator.lt, DateUtil.getSystemTimeDay(DateUtil.addDay(totalEndTime))));
        }
