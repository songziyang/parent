...
<script>
	...
	$(function(){		
		var isSend = '<c:out value="${invest.isSend}"/>';
		change(isSend);
	});
...
<body>
	...
	<div class="page-header position-relative">
		<div class="header-title">
			<h1>
				<c:choose>
					<c:when test="${!empty redpaper.id}">体验金编辑</c:when>
				...
				<div class="widget">
					...
					<div class="widget-body">
						<div id="registration-form">
							<form
								action="${ctx}/redpapermanager/invest/saveInvest"
								method="post" role="form" id="editForm" name="editForm">

								<input type="hidden" name="id" value="${invest.id}" />


								<div class="form-title">体验金信息</div>
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>体验金份数<sup
													style="color: red;">&nbsp;*</sup>：
											</label> <input type="text" class="form-control" tabindex="1"
												name="copies"
												value="<c:out value="${invest.copies}"/>"
									...
									<div class="col-sm-6">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>天数<sup
													style="color: red;">&nbsp;*</sup>：
											</label> <input type="text" class="form-control" tabindex="1"
												value="<c:out value="${invest.days}"/>"
								...
								<div class="row">
									<div class="col-sm-6">
										<label style="color: #999;">是否发送短信<sup
											style="color: red;">&nbsp;*</sup>：
										</label>
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label><sup
													style="color: red;">&nbsp;</sup></label> <select
												id="isSend" name="isSend" class="form-control" required onchange="javascript:changeViews();">
													<option value="">--请选择--</option>
													<option value="0"
														<c:if test="${invest.isSend eq 0 }">selected="selected"</c:if>>是</option>
													<option value="1"
														<c:if test="${invest.isSend eq 1 }">selected="selected"</c:if>>否</option>
												</select>
											</span>
										</div>
									</div>
										...
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>短信内容：</label>
												<textarea style="padding-left: 120px;" class="form-control"
													...
													data-bv-stringlength-message="不能超过500个字"><c:out value="${invest.content}"/></textarea>
											</span>
										</div>
									</div>
								</div>
								<div style="text-align: right; margin-bottom: 10px;">
									...
									<a href="${ctx}/redpapermanager/invest/listInvest"
