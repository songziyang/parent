...
<body>
	...
	<div class="page-header position-relative">
		...
		<div class="header-buttons">
			<a class="sidebar-toggler" href="#"> <i class="fa fa-arrows-h"></i>
				...
				href="${ctx}/userinfo/benefits/listBenefits"> <i
	...
	<!-- /Page Header -->
	<!-- Page Body -->
	<form action="${ctx}/userinfo/benefits/listBenefits" method="post"
				...
				<div class="col-xs-12 col-md-12">
					...
					<div class="widget">
						<div class="widget-header  with-footer">
							<span class="widget-caption">用户收益</span>
							<div class="widget-buttons">
						...
						<div class="widget-body">
							<div class="flip-scroll">
								<div
									...
									收益总计：
									<c:out value="${empty totalFund ? 0 : totalFund}" />
								</div>
									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${page.content}" var="platform"
											...
											<tr>
												...
												<td>
													<c:choose>
														<c:when test="${platform.user.userLeve.gradeNum eq 1}"><img src="${ctx}/static/lib/images/vip1.png" /></c:when>
														<c:when test="${platform.user.userLeve.gradeNum eq 2}"><img src="${ctx}/static/lib/images/vip2.png" /></c:when>
													</c:choose>
													<a href="${ctx}/userinfo/user/listUserInfo/<c:out value='${platform.user.id}'/>"
													   data-simpletooltip="init" title='<c:out value="${platform.user.remark}" />'
													   <c:if test="${not empty platform.user.remark}">style='color: red;'</c:if>>
														<c:out value="${platform.user.username}" />
													</a>
												</td>
												<td>
													<a href="${ctx}/userinfo/user/listUserInfo/<c:out value='${platform.user.id}'/>"
													   data-simpletooltip="init" title='<c:out value="${platform.user.remark}" />'
													   <c:if test="${not empty platform.user.remark}">style='color: red;'</c:if>>
														<c:out value="${platform.user.mobile}" />
													</a>
												</td>
