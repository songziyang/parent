							...
							<tbody style="font-size: 12px;">
								<c:forEach items="${page.content}" var="redpackage"
									...
									<tr>
										...
										<td>
											<c:choose>
												<c:when test="${redpackage.state eq 0 }">
													未领用
												</c:when>
												<c:when test="${redpackage.state eq 1 }">
													未使用
												</c:when>
												<c:when test="${redpackage.state eq 3 }">
													已使用
												</c:when>
												<c:when test="${redpackage.state eq 4 }">
													已过期
												</c:when>
												<c:when test="${redpackage.state eq 5 }">
													现金领取
												</c:when>
											</c:choose>
										</td>
