	...
	function checkradio() {
		...
		if (len > 0) {
			...
			if (result > 0) {
				...
				var address = "${ctx}/redpacket/userRedpacket/sendRedpacket?id=" + id;
				bootbox.dialog({
					message : "您确定要发送此红包么？",
					title : "红包发放提示",
...
<body>
	...
	<div class="page-header position-relative">
		...
		<div class="header-buttons">
			...
			<a class="refresh" id="refresh-toggler" href="${ctx}/redpacket/userRedpacket/send">
	...
	<!-- /Page Header -->
	<!-- Page Body -->
	<form action="${ctx}/redpacket/userRedpacket/querySendUserCount"
									...
									<div class="form-inline" role="form">
										...
										<div class="form-group">
											注册日期&nbsp;<input type="text" name="registerStartDate"
															 ...
															 tabindex="1" style="padding-left: 10px;width: 120px;" maxlength="40"
											...
											<input type="text" name="registerEndDate"
												   ...
												   style="padding-left: 10px;width: 120px;" maxlength="40">
										</div>
									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${redpackets}" var="redpacket" varStatus="status">
											<tr>
												...
												<td><c:out value="${redpacket.productType.type}" /></td>
												<td><c:out value="${redpacket.productDays}" /></td>
												<td>
													<c:choose>
														...
														<c:when test="${redpacket.triggerType eq 12}">定存排行活动</c:when>
														<c:when test="${redpacket.triggerType eq 14}">周年活动加息券</c:when>
													</c:choose>
												</td>
												<td>
													<c:choose>
														<c:when test="${redpacket.redpacketType eq 1}">活期宝加息</c:when>
														<c:when test="${redpacket.redpacketType eq 2}">定存宝加息</c:when>
													</c:choose>
												</td>
												<td><c:out value="${redpacket.useDays}" /></td>
												<td><c:out value="${redpacket.investDays}" /></td>
												<td><c:out value="${redpacket.giveValue}" /></td>
												<td>
													<fmt:formatDate value="${redpacket.activityBeginDate}"  pattern="yyyy年MM月dd日" />
												</td>
												<td>
													<fmt:formatDate value="${redpacket.activityFinishDate}"  pattern="yyyy年MM月dd日" />
												</td>
											</tr>
										</c:forEach>
									</tbody>
