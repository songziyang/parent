	...
	function checkradio() {
		...
		if (len > 0) {
			...
			if (result > 0) {
				...
				var address = "${ctx}/redpacket/voucherSend/sendRedpacket?id=" + id;
				bootbox.dialog({
					message : "您确定要发送此红包么？",
					title : "红包发放提示",
...
<body>
	...
	<div class="page-header position-relative">
		...
		<div class="header-buttons">
			...
			<a class="refresh" id="refresh-toggler" href="${ctx}/redpacket/voucherSend/send">
	...
	<!-- /Page Header -->
	<!-- Page Body -->
	<form action="${ctx}/redpacket/voucherSend/querySendUserCount"
									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${redpackets}" var="redpacket" varStatus="status">
											<tr>
												...
												<td><c:out value="${redpacket.limitFund}" /></td>
												<td>
													<fmt:formatDate value="${redpacket.beginDate}"  pattern="yyyy年MM月dd日" />
												</td>
												<td>
													<fmt:formatDate value="${redpacket.endDate}"  pattern="yyyy年MM月dd日" />
												</td>
											</tr>
										</c:forEach>
									</tbody>
