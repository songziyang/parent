...
<body>
	...
	<div class="page-header position-relative">
		<div class="header-title">
			<h1>
				<c:choose>
					<c:when test="${!empty mslever.id}">配资利息率信息编辑</c:when>
					<c:otherwise>配资利息率信息添加</c:otherwise>
				</c:choose>
			</h1>
		</div>
		<!--Header Buttons-->
				...
				<div class="widget">
					...
					<div class="widget-body">
						<div id="registration-form">
							<form action="${ctx}/matchstock/inter/saveInter"  id="editForm" name="editForm" method="post" role="form">

								<input type="hidden" name="id"
									value='<c:out value="${msinter.id}"/>'> 
						
								<input
									type="hidden" name="createuser"
									value='<c:out value="${msinter.createuser}"/>'>
								<input
									type="hidden" name="created"
									value='<c:out value="${msinter.created}"/>'>
			
									<input
									type="hidden" name="updateuser"
									value='<c:out value="${msinter.updateuser}"/>'>
									
								<div class="form-title">配资利息率信息</div>


									<div class="col-sm-12">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>配资利息率信息<sup
													style="color: red;">&nbsp;*</sup>：
											</label> <input type="text" class="form-control" tabindex="1"
												style="padding-left: 120px;" name="interestrate" required
												value="<c:out value="${msinter.interestrate}"/>" maxlength="7"
												data-bv-stringlength-max="7"
												data-bv-stringlength-message="不能超过7个字" data-bv-regexp="true"
												 data-bv-regexp-regexp="^\d+(\.\d+)?$"
												data-bv-regexp-message="利息率格式错误">
											</span>
										</div>
									</div>
									<!--  							<div class="row">
									<div class="col-sm-12">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>配资利息率名称</label>
								...
								<div style="text-align: right; margin-bottom: 10px;">
									...
									<a href="${ctx}/matchstock/inter/listinter"
