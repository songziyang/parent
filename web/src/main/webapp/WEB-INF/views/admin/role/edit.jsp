...
<body>
	...
	<div class="page-header position-relative">
		<div class="header-title">
			<h1>
				<c:choose>
					<c:when test="${!empty role.id}">角色编辑</c:when>
					<c:otherwise>角色添加</c:otherwise>
				</c:choose>
			</h1>
		</div>
		<!--Header Buttons-->
				...
				<div class="widget">
					...
					<div class="widget-body">
						<div id="registration-form">
							<form action="${ctx}/admin/role/saveRole" id="editForm"
								name="editForm" method="post" role="form" >
								 ...
								 <input type="hidden" name="id" value='<c:out value="${role.id}"/>'   >
        							 ...
        							 <input type="hidden" name="purviews" value='<c:out value="${role.purviews}"/>'   >

								<div class="form-title">角色信息</div>
								<div class="row">
									<div class="col-sm-12">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>角色名称<sup style="color: red;">&nbsp;*</sup>：</label>
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>角色描述</label>
												<textarea style="padding-left: 100px;" class="form-control"
													rows="4" name="remark" maxlength="100"
													data-bv-stringlength-max="100"
													data-bv-stringlength-message="不能超过100个字"><c:out value="${role.remark}"/></textarea>
											</span>
										</div>
									</div>
								</div>
								<div style="text-align: right; margin-bottom: 10px;">
									...
									<a href="${ctx}/admin/role/listRole"
