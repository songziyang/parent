...
<body>
	...
	<div class="page-header position-relative">
		...
		<div class="header-buttons">
			...
			</a> <a class="refresh" id="refresh-toggler" href="${ctx}/admin/admin/listAdmin"> <i
	...
	<!-- /Page Header -->
	<!-- Page Body -->
	<form action="${ctx}/admin/admin/listAdmin" method="post" id="listForm"
		name="listForm">
		<div class="page-body">
			<div class="row">
							...
							<div class="widget collapsed">
								...
								<div class="widget-body">
									<div class="form-inline" role="form">
										...
										<div class="form-group">
											<div>
												<label class="control-label no-padding-right">&nbsp;所属角色</label>
												<select id="roleId"  class="form-control" name="roleId" style="width: 200px;">
													<option value="">全部</option>
													<c:forEach items="${roleLst }" var="roles">
														<option value='<c:out value="${roles.id }" />'><c:out
																value="${roles.name }" />
														</option>
													</c:forEach>
												</select>
									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${page.content}" var="admin"
											...
											<tr>
												...
												<td><c:out value="${admin.realname}" /></td>
												<td><c:out value="${admin.role.name}" /></td>
												<td><fmt:formatDate value="${admin.createDate}"
														pattern="yyyy年MM月dd日 HH时mm分" /></td>
												<td width="20"><label> <input type="checkbox"
														name="ids" value='<c:out value="${admin.id}" />'>
														<span class="text"></span>
												</label></td>
												<td width="200" align="center"><shiro:hasPermission
														name="admin_edit">
														<a
															href='${ctx}/admin/admin/editAdmin/<c:out value="${admin.id}" />'
														...
														<a class="btn btn-danger btn-xs delete"
															onclick="removeData('${ctx}/admin/admin/deleteAdmin/','<c:out value="${admin.id}" />')"><i
