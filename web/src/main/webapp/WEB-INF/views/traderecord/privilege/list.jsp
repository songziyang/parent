...
<head>
    ...
    <script type="text/javascript">
        ...
        function exprotExcel(condition) {
            var url = "${ctx}/traderecord/privilege/exportExcel/" + condition
...
<div class="page-header position-relative">
    ...
    <div class="header-buttons">
        ...
        </a> <a class="refresh" id="refresh-toggler" href="${ctx}/traderecord/privilege/listPrivilegeRecored"> <i
...
<!-- /Page Header -->
<!-- Page Body -->
<form action="${ctx}/traderecord/privilege/listPrivilegeRecored"
            ...
            <div class="col-xs-12 col-md-12">
                ...
                <div class="widget">
                    <div class="widget-header  with-footer">
                        <span class="widget-caption">新手宝交易记录</span>
                    ...
                    <div class="widget-body">
                        <div class="flip-scroll">
                            ...
                            <table class="table table-hover table-striped table-bordered">
                                ...
                                <c:forEach items="${page.content}" var="privilegerecored" varStatus="status">
                                    <tr>
                                        <td class="table_no" width="80" align="center"></td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${privilegerecored.user.userLeve.gradeNum eq 1}"><img src="${ctx}/static/lib/images/vip1.png" /></c:when>
                                                <c:when test="${privilegerecored.user.userLeve.gradeNum eq 2}"><img src="${ctx}/static/lib/images/vip2.png" /></c:when>
                                            </c:choose>
                                            <a href='${ctx}/userinfo/user/listUserInfo/<c:out value="${privilegerecored.user.id}" />'
                                               data-simpletooltip="init" title='<c:out value="${privilegerecored.user.remark}" />'
                                               <c:if test="${not empty privilegerecored.user.remark}">style='color: red;'</c:if>>
                                                <c:out value="${privilegerecored.user.username}"/>
                                            </a>
                                        </td>
                                        <td>
                                            <a href='${ctx}/userinfo/user/listUserInfo/<c:out value="${privilegerecored.user.id}" />'
                                               data-simpletooltip="init" title='<c:out value="${privilegerecored.user.remark}" />'
                                               <c:if test="${not empty privilegerecored.user.remark}">style='color: red;'</c:if>>
                                                <c:out value="${privilegerecored.user.mobile}"/>
                                            </a>
                                        </td>
                                        <td>
                                            <c:out value="${privilegerecored.names}"/>
                                        </td>

                                        <td>
                                            <c:choose>
                                                <c:when test="${privilegerecored.type eq 1}">
                                                    购买
                                                </c:when>
                                                <c:when test="${privilegerecored.type eq 2}">
                                                    赎回
                                                </c:when>
                                            </c:choose>
                                        </td>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${privilegerecored.fundSource eq 0}">
                                                    资金余额
                                                </c:when>
                                                <c:when test="${privilegerecored.fundSource eq 1}">
                                                    收益
                                                </c:when>
                                                <c:when test="${privilegerecored.fundSource eq 2}">
                                                    到期本息
                                                </c:when>
                                            </c:choose>
                                        </td>
                                        ...
                                        <td>
                                            <c:out value="${privilegerecored.fund}"/>
                                        </td>
                                        <td>
                                            <fmt:formatDate value="${privilegerecored.buyDate}"
