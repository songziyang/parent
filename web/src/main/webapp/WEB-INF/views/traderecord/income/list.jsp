...
<head>
    ...
    <script type="text/javascript">
        ...
        function exprotExcel(condition) {
            var url = "${ctx}/traderecord/income/export/" + condition
...
<div class="page-header position-relative">
    ...
    <div class="header-buttons">
        ...
        </a> <a class="refresh" id="refresh-toggler" href="${ctx}/traderecord/income/list"> <i
...
<!-- /Page Header -->
<!-- Page Body -->
<form action="${ctx}/traderecord/income/list"
            ...
            <div class="col-xs-12 col-md-12">
                                ...
                                <div class="form-inline" role="form">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="ptype">
                                            ...
                                            <option value="4">砸蛋活动</option>
                                            <option value="5">定存返现</option>
                                            <option value="6">基金收益</option>
                                            <option value="7">采摘返现</option>
                                            <option value="8">网袋还款</option>
                                            <option value="9">代金券红包</option>
                                            <option value="10">随心存</option>
                                        </select>
                                    </div>
                ...
                <div class="widget">
                    <div class="widget-header  with-footer">
                        <span class="widget-caption">用户收益记录</span>
                            ...
                            <table class="table table-hover table-striped table-bordered">
                                ...
                                <c:forEach items="${page.content}" var="record" varStatus="status">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${record.ptype eq 1}">
                                                    活期宝
                                                </c:when>
                                                <c:when test="${record.ptype eq 2}">
                                                    定存宝
                                                </c:when>
                                                <c:when test="${record.ptype eq 3}">
                                                    现金红包
                                                </c:when>
                                                <c:when test="${record.ptype eq 4}">
                                                    砸蛋活动
                                                </c:when>
                                                <c:when test="${record.ptype eq 5}">
                                                    定存返现
                                                </c:when>
                                                <c:when test="${record.ptype eq 6}">
                                                    基金收益
                                                </c:when>
                                                <c:when test="${record.ptype eq 7}">
                                                    采摘返现
                                                </c:when>
                                                <c:when test="${record.ptype eq 8}">
                                                    网袋还款
                                                </c:when>
                                                <c:when test="${record.ptype eq 9}">
                                                    代金券红包
                                                </c:when>
                                                <c:when test="${record.ptype eq 10}">
                                                    随心存
                                                </c:when>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <fmt:formatDate value="${record.opdate}" pattern="yyyy年MM月dd日 HH时mm分"/>
                                        </td>
