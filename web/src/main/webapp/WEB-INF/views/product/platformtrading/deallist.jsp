									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${page.content}" var="deal"
											...
											<tr>
												...
												<td><fmt:formatNumber value="${deal.fund}"
														type="number" pattern="0.00" maxFractionDigits="2" /></td>
												<td>
													<c:if test="${deal.platformTrading.type eq 1}">
														充值
													</c:if>
													<c:if test="${deal.platformTrading.type eq 2}">
														提现
													</c:if>
													<c:if test="${deal.platformTrading.type eq 3}">
														活期宝
													</c:if>
													<c:if test="${deal.platformTrading.type eq 4}">
														定存宝
													</c:if>
													<c:if test="${deal.platformTrading.type eq 5}">
														机构宝
													</c:if>
													<c:if test="${deal.platformTrading.type eq 6}">
														私人订制
													</c:if>
													<c:if test="${deal.platformTrading.type eq 7}">
														随心存
													</c:if>
												</td>
												<td><fmt:formatDate value="${deal.operationDate}"  pattern="yyyy年MM月dd日 " /></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
