...
<body>
	...
	<div class="page-header position-relative">
		...
		<div class="header-buttons">
			...
			</a> <a class="refresh" id="refresh-toggler" href="${ctx}/product/institution/listMailuo"> <i
	...
	<!-- /Page Header -->
	<!-- Page Body -->
	<form action="${ctx}/product/institution/listMailuo" method="post"
				...
				<div class="col-xs-12 col-md-12">
					...
					<div class="widget">
						<div class="widget-header  with-footer">
							<span class="widget-caption">麦多宝购买记录</span>
							<div class="widget-buttons">
							...
							<div class="flip-scroll">
								...
								<table class="table table-hover table-striped table-bordered">
									<thead style="font-size: 16px; font-weight: bold;">
										<tr>
											<th width="80" style="text-align: center;" rowspan="2">序号</th>
											<th rowspan="2">产品名称</th>
											<th rowspan="2">产品天数</th>
											<th rowspan="2">年化利率(%)</th>
											<th rowspan="2">购买日期</th>
											<th rowspan="2">购买金额</th>
											<th rowspan="2">到期时间</th>
											<th rowspan="2">状态</th>
											<th colspan="5" style="text-align: center;" >收益</th>
									...
									<tbody style="font-size: 12px;">
										<c:forEach items="${page.content}" var="ragularAccount"
											...
											<tr>
												...
												<c:set var="productDays" value="${ragularAccount.days}" />
												<c:set var="revenue" value="0"></c:set>
												<c:forEach var="ragularRefund" items="${ragularAccount.ragularRefund}">
													<c:if test="${revenue eq 0}">
														<c:set var="revenue" value="${ragularRefund.fund}"></c:set>
													</c:if>
												</c:forEach>
												<c:choose>
													<%--如果是3个月产品 --%>
													<c:when test="${productDays eq 90}">
														<td>
														<c:out value="${revenue}"/></td>
														<td>-</td>
														<td>-</td>
														<td>-</td>
													</c:when>
													<%--如果是6个月产品 --%>
													<c:when test="${productDays eq 180}">
														<td>
															<c:out value="${revenue}"/>
														</td>
														<td>
