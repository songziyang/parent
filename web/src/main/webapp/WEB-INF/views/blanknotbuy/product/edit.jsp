			...
			$.post(url, function(data, status) {
				...
				$(obj).each(
						function(index) {
							...
							var val = obj[index];
							$("#ct1").append(
									'<p"><a href=javaScript:setUserNameAndUID('
											+ "'" + val.userName + "','"
											+ val.userID + "'"
											+ ');  style="color:green;"' + '>'
											+ '用户昵称: '+val.userName + '</a>&nbsp&nbsp可用白条余额:'+val.usableMoney+'</p>');
						});
			});
		}
	}
							...
							<form action="${ctx}/blanknotbuy/productlist/buyproduct"
									...
									<div class="col-sm-6">
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>购买金额<sup
												...
												maxlength="6" data-bv-stringlength-max="10" required
										...
										<div class="form-group">
											<span class="form-group input-icon icon-right"> <label>购买日期<sup
												...
												value="<fmt:formatDate value="${blanknote.createTime}"
														pattern="yyyy-MM-dd" />"
												onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'buyDate\')}',skin:'twoer',dateFmt:'yyyy-MM-dd',readOnly:true,onpicked:function(dp){$('#editForm').data('bootstrapValidator').updateStatus(this.name, 'NOT_VALIDATED').validateField(this.name);}})">
											</span> </span>
										</div>
									</div>
								</div>
								...
								<div style="text-align: right; margin-bottom: 10px;">
									<shiro:hasPermission name="productbuy_purchase">
										<button type="submit" class="btn btn-labeled btn-success" style="width: 100px">
											<i class="btn-label glyphicon glyphicon-ok"></i>购买
										</button>
									</shiro:hasPermission>
