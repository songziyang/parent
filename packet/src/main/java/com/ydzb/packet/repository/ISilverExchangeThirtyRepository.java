 ...
 * 银币活动
 */
public interface ISilverExchangeThirtyRepository extends IBaseRepository<SilverExchangeThirty, Long> {
     ...
     */
    @Modifying
    @Query(" UPDATE SilverExchangeThirty SET status = :targetStatus, sended = :sended WHERE user.id = :userId AND status = :sourceStatus AND type = :type")
     ...
     */
    @Modifying
    @Query(" UPDATE SilverExchangeThirty SET status = :status, sended = :sended WHERE id = :id")
